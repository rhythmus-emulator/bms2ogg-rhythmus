...
MidSong* mid_song_for_stream(MidSongOptions *options)
{
  MidSong *song;
  int i;
  ...
  // allocating fake event note should be compatible with mid_song_free
  song->events = (MidEvent *) timi_calloc(sizeof(MidEvent));
  song->events->time = INT_MAX;
  song->events->type = ME_EOT;
